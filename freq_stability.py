...
for linewidth in LINEWIDTHS:
    SAVE_FOLDER=rf'{DATA_FOLDER}\Freq_stability\{linewidth}nm'
    for wavelength in tqdm(WAVELENGTHS, desc=f"Processing LW {linewidth}nm"):
        
        # Формирование имя файлов
        filename=f'{wavelength}nm_{linewidth}nm'
        png_title=f'LW={linewidth}nm, WL={wavelength}nm'
                
        iteration_arr, max_freq_arr = [], []
        
        for iteration in range(1, NUMBER_ITERATIONS+1):
            
            rf_txt_file=rf"{DATA_FOLDER}\RF_port_1064nm\{linewidth}nm\{wavelength}nm\span_1000.0MHz\RF_{iteration}_{wavelength}nm_{linewidth}nm_span_1000.0MHz.txt"
